...
<xsl:stylesheet 
  ...
  <xsl:template match="v3:td">
    <xsl:text>@td{</xsl:text> <xsl:value-of select="normalize-space(.)"/> <xsl:text>}</xsl:text>
  </xsl:template>

  <xsl:template match="v3:th">
    <xsl:text>@th{</xsl:text> <xsl:value-of select="normalize-space(.)"/> <xsl:text>}</xsl:text>
  </xsl:template>

  <xsl:template match="v3:td[descendant::v3:content]">
    <xsl:text>@td[descendant::v3:content]{</xsl:text> <xsl:value-of select="normalize-space(.)"/> <xsl:text>}</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="v3:item">
     <xsl:text>@item{</xsl:text> <xsl:value-of select="normalize-space(.)"/> <xsl:text>}</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="/v3:document/v3:component/v3:structuredBody/v3:component/v3:section[descendant::v3:code[@code='34066-1']]" priority="1">
    BOXED WARNING: <xsl:value-of select="v3:title"/> <xsl:text>&#xa;</xsl:text>
